...
flow_checkpoint_path = './models/hmdb51/hmdb51-flow.ckpt'
...
def forward_one_epoch(net, feature_fg_5c, target, training=True, mode='clf'):
    if training:
...
def validate_flow(val_loader, net):
    ...
    with tqdm.tqdm(val_loader, total=len(val_loader), ncols=0) as pbar:
        for n_iter, (input, target) in enumerate(pbar):
            ...
            output= forward_one_epoch(net, feat_dict['Mixed_5c'], target_var, training=False)
            output_list.append(output.data)
...
if __name__ == '__main__':
    ...
    dict_trained = torch.load(rgb_checkpoint_path)
    dict_new = rgb_net.state_dict().copy()
    new_list = list(rgb_net.state_dict().keys())
    trained_list = list(dict_trained.keys())
    for i in range(0,344):
        dict_new[new_list[i]] = dict_trained[trained_list[i]]
    rgb_net.load_state_dict(dict_new)
    ...
    rgb_split_file = os.path.join("./datasets/settings/hmdb51", rgb_setting_file)
    if not os.path.exists(rgb_split_file):
        print("No split file exists in %s directory. Preprocess the dataset first" % ("./datasets/settings/hmdb51"))

    rgb_dataset = datasets.__dict__['hmdb51'](root="/Datasets/hmdb51_frames",
    ...
    flow_split_file = os.path.join("./datasets/settings/hmdb51", flow_setting_file)
    if not os.path.exists(flow_split_file):
        print("No split file exists in %s directory. Preprocess the dataset first" % ("./datasets/settings/hmdb51"))

    flow_dataset = datasets.__dict__['hmdb51'](root="/Datasets/hmdb51_frames",
